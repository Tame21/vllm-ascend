...
QWEN3_5 = "qwen3_5"
MAGISTRAL = "magistral"
NATIVE_LORA_RANKS = frozenset({8, 16, 32, 64})
...
def validate_config(config) -> None:
    ...
    if config.lora_config.max_lora_rank not in NATIVE_LORA_RANKS:
        raise ValueError(
            f"The {model_name} native Ascend LoRA kernels require "
            "max_lora_rank in {8, 16, 32, 64}"
        )
