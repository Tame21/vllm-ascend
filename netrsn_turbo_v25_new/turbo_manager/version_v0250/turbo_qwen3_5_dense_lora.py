...
def apply_qwen3_5_dense_lora_patch() -> None:
    ...
    TurboManager.register_patch(
        ...
        punica_patch.wrap_update_metadata(PunicaWrapperNPU.update_metadata),
    )
    for name in (
        "add_shrink",
        "add_expand",
    ...
    TurboManager.register_patch(
        ...
        model_manager_patch.wrap_init(LoRAModelManager.__init__),
    )
    TurboManager.register_patch(
        "vllm.lora.worker_manager.WorkerLoRAManager._load_adapter",
        worker_manager_patch.wrap_load_adapter(WorkerLoRAManager._load_adapter),
    )
    TurboManager.register_patch(
