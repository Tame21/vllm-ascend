...
class TestC8MXFPFullGraph(TestBase):
    def test_new_cache_reinitializes_static_v_scale(self):
        impl = object.__new__(AscendC8MXFPAttentionBackendImpl)
        impl.num_kv_heads = 1
        impl.head_size = 64
        impl.save_v_scale_flag = True

        profile_v_scale_cache = torch.zeros(1, 1, 1, 64, 2, dtype=torch.uint8)
        impl._v_scale_cache_ref = lambda: profile_v_scale_cache

        key_cache = torch.zeros(1, 64, 1, 64, dtype=torch.float8_e4m3fn)
        value_cache = torch.zeros_like(key_cache)
        key_scale_cache = torch.zeros(1, 1, 64, 1, 2, dtype=torch.uint8)
        real_v_scale_cache = torch.zeros_like(profile_v_scale_cache)
        metadata = SimpleNamespace(slot_mapping=torch.tensor([0]))

        with (
            patch("vllm_ascend.attention.attention_v1.DeviceOperator.reshape_and_cache"),
            patch("vllm_ascend.attention.attention_v1.scatter_mxfp_k_scale_cache"),
        ):
            impl.reshape_and_cache(
                torch.zeros(1, 1, 64, dtype=torch.float8_e4m3fn),
                torch.zeros(1, 1, 64, dtype=torch.float8_e4m3fn),
                torch.zeros(1, 1, 1, 2, dtype=torch.uint8),
                torch.ones(64, dtype=torch.uint8),
                (key_cache, value_cache, key_scale_cache, real_v_scale_cache),
                metadata,
            )

        self.assertTrue(torch.all(real_v_scale_cache == 1))
        self.assertIs(impl._v_scale_cache_ref(), real_v_scale_cache)
