...
class AscendAutoRegressiveSpeculator(AutoRegressiveSpeculator):
    ...
    def propose(
        ...
        if (
            ...
            and self.attn_architecture not in ("DSA", "SFA")
        ):
    ...
    def _copy_num_rejected_to_cpu(
        self, num_rejected: torch.Tensor, num_reqs: int
    ) -> None:
        ...
        self.num_rejected_cpu[:num_reqs].copy_(
            num_rejected[:num_reqs], non_blocking=False
        )
